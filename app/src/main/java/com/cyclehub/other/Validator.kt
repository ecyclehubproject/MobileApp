...
object Validator {
    ...
    private val EMPTY_FIELD = "This field cannot empty"
     ...
     * @return - true if the phone is valid.
     */
    fun isEmpty(data: Any, updateUI: Boolean = true): Boolean {
        val str = getText(data)
        val valid = str.isNotEmpty()

        // Set error if required
        if (updateUI) {
            val error: String? = if (valid) null else EMPTY_FIELD
            setError(data, error)
        }
    ...
    fun isEmptyWithToast(view: View, data: Any, updateUI: Boolean = true): Boolean {
        val str = getText(data)
        val valid = str.isNotEmpty()
